  ...
  return (
    <div className="product-card animate-cardFadeUp group bg-card rounded-xl border overflow-hidden cursor-pointer" onClick={handleClick}>
      ...
      <div className="p-2 sm:p-3 space-y-1 sm:space-y-1.5">
        <p className="text-[10px] sm:text-xs text-[#C5A028] font-semibold truncate" style={FB}>{product.brand}</p>
        <p className="text-xs sm:text-sm font-medium line-clamp-2 leading-snug min-h-[2rem] sm:min-h-[2.5rem] text-slate-800 dark:text-slate-100" style={FB}>{product.name}</p>
        <div className="flex items-center gap-1 sm:gap-1.5"><StarRating rating={product.rating} size={10}/><span className="text-[10px] sm:text-xs text-muted-foreground">({product.reviews})</span></div>
        <PriceDisplay price={product.price} oldPrice={product.oldPrice}/>
        <Button size="sm" className="w-full mt-0.5 sm:mt-1 bg-gradient-to-r from-[#006233] to-[#00A651] hover:from-[#004D25] hover:to-[#006233] text-white border-0 font-semibold text-xs sm:text-sm h-7 sm:h-8" onClick={handleCart}><ShoppingCart size={12} className="mr-1 sm:w-[14px]"/> Add to Cart</Button>
      </div>
    </div>
  );
}
...
function HomeView({ onQuickView, navigate }: { onQuickView: (p: Product) => void; navigate: (h: string) => void }) {
  ...
  return (
    <div className="animate-fadeUp">
      ...
      <div className="relative overflow-hidden rounded-xl sm:rounded-2xl mb-6 sm:mb-8 shadow-2xl" style={{ minHeight: 280 }}>
        {heroSlides.map((slide, i) => {
          ...
          return (
            <div key={i} className={`absolute inset-0 transition-opacity duration-700 ${i === heroIdx ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
              ...
              <div className="absolute inset-0 flex items-center p-4 sm:p-6 md:p-14">
                <div className="max-w-xl text-white space-y-3 sm:space-y-5 animate-heroTextReveal">
                  <div className="inline-flex items-center gap-1.5 sm:gap-2 bg-white/15 backdrop-blur-sm px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium border border-white/20"><span className="text-[#FFD700] sm:text-lg">&#9734;</span> Bachat Bazar</div>
                  <h2 className="text-xl sm:text-3xl md:text-5xl font-bold drop-shadow-lg leading-tight" style={FH}>{title}</h2>
                  <p className="text-sm sm:text-base md:text-xl opacity-95 drop-shadow font-medium leading-relaxed line-clamp-2 sm:line-clamp-none" style={FB}>{sub}</p>
                  <Button className="bg-[#C5A028] hover:bg-[#B08D20] text-white font-bold px-5 sm:px-8 py-2 sm:py-3 shadow-lg text-sm sm:text-base rounded-xl" onClick={() => navigate(ctaLink)}>{cta} <ChevronRight size={16} className="ml-1"/></Button>
                </div>
              </div>
            </div>
          );
        })}
        <button onClick={() => setHeroIdx(i => (i - 1 + heroSlides.length) % heroSlides.length)} className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 bg-white/20 backdrop-blur rounded-full p-2 sm:p-3 text-white hover:bg-white/40 transition shadow z-20"><ChevronLeft size={16}/></button>
        <button onClick={() => setHeroIdx(i => (i + 1) % heroSlides.length)} className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 bg-white/20 backdrop-blur rounded-full p-2 sm:p-3 text-white hover:bg-white/40 transition shadow z-20"><ChevronRight size={16}/></button>
        <div className="absolute bottom-3 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 sm:gap-2 z-20">{heroSlides.map((_, i) => <button key={i} onClick={() => setHeroIdx(i)} className={`w-3 h-3 rounded-full transition shadow backdrop-blur ${i === heroIdx ? 'bg-[#FFD700] scale-125' : 'bg-white/40'}`}/>)}</div>
      </div>
      ...
      {/* Promo Strip */}
      <div className="mb-6 sm:mb-8 bg-gradient-to-r from-[#006233] via-[#00A651] to-[#006233] rounded-xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-white shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3"><span className="text-xl sm:text-2xl">&#127477;&#127472;</span><div><p className="font-bold text-xs sm:text-sm md:text-base" style={FH}>Pakistan Zindabad! Free Delivery Nationwide</p><p className="text-[10px] sm:text-xs opacity-80">On orders above Rs 25,000 — Cash on Delivery Available</p></div></div>
        <Button className="bg-[#C5A028] hover:bg-[#B08D20] text-white font-bold text-xs sm:text-sm shrink-0 shadow w-full sm:w-auto" onClick={() => navigate(makeHash('shop'))}>Shop Now</Button>
      </div>

      {/* ─── CATEGORIES — CLEARLY VISIBLE ─── */}
      <section className="mb-8 sm:mb-10">
        <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-5 flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><Store size={20} className="text-[#006233]"/> Shop by Category</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
          {s.categories().map(cat => {
            const Icon = getCatIcon(cat.icon);
            return (
              <button key={cat.id} onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); navigate(makeHash('shop', undefined, { category: cat.id })); }} className="category-card group relative rounded-xl sm:rounded-2xl overflow-hidden cursor-pointer shadow-lg hover:shadow-xl ring-1 ring-black/5 dark:ring-white/10 aspect-[3/2]">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-2 sm:p-3 z-10">
                  <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-white/25 backdrop-blur-sm flex items-center justify-center mb-2 sm:mb-3 shadow-lg ring-1 ring-white/30"><Icon size={20} className="drop-shadow-lg text-white sm:w-[26px] sm:h-[26px]"/></div>
                  <span className="text-xs sm:text-base font-bold text-center drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)]" style={FB}>{cat.name}</span>
                </div>
              </button>
            );
          })}
      ...
      {/* Flash Sale */}{flashItems.length > 0 && (<section className="mb-8 sm:mb-10"><div className="flex items-center justify-between mb-3 sm:mb-4"><h3 className="text-lg sm:text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><Flame size={20} className="text-red-500"/> Flash Sale</h3><div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm bg-red-50 dark:bg-red-900/20 px-2 sm:px-3 py-1 rounded-full"><Clock size={14} className="text-red-500"/> Ends in <CountdownTimer targetDate={flashSaleEnd}/></div></div><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3">{flashItems.map(p => <ProductCard key={p.id} product={p} onQuickView={onQuickView}/>)}</div></section>)}
      {/* Featured */}{featured.length > 0 && (<section className="mb-8 sm:mb-10"><div className="flex items-center justify-between mb-3 sm:mb-4"><h3 className="text-lg sm:text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><Award size={20} className="text-[#C5A028]"/> Featured</h3><Button variant="ghost" size="sm" onClick={() => navigate(makeHash('shop', undefined, { featured: 'true' }))} className="text-[#006233] dark:text-[#00A651] text-xs sm:text-sm">View All <ChevronRight size={14}/></Button></div><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">{featured.map(p => <ProductCard key={p.id} product={p} onQuickView={onQuickView}/>)}</div></section>)}
      {/* Eid Banner */}<div className="mb-8 sm:mb-10 rounded-xl bg-gradient-to-r from-[#006233] to-[#004D25] p-4 sm:p-6 md:p-10 text-white flex flex-col md:flex-row items-center gap-4 sm:gap-6 shadow-xl pk-pattern relative overflow-hidden"><div className="absolute top-4 right-4 text-4xl sm:text-6xl opacity-10" style={{ fontFamily: 'serif' }}>&#9734;</div><div className="flex-1 space-y-1 sm:space-y-2"><p className="text-[#FFD700] text-xs sm:text-sm font-bold uppercase tracking-wider" style={FB}>Limited Time Offer</p><h3 className="text-xl sm:text-2xl md:text-3xl font-bold" style={FH}>Eid Mubarak Sale — 25% Off!</h3><p className="text-sm sm:text-base opacity-90 font-medium" style={FB}>Use code <span className="font-mono bg-[#C5A028] px-2 py-0.5 rounded text-white text-xs sm:text-sm">EID25</span> on orders above Rs 50,000</p></div><Button className="bg-[#C5A028] hover:bg-[#B08D20] text-white font-bold shrink-0 shadow-lg text-sm sm:text-base w-full sm:w-auto" onClick={() => navigate(makeHash('deals'))}>Shop Deals <ArrowRight size={16}/></Button></div>
      {/* Trending */}{trending.length > 0 && (<section className="mb-8 sm:mb-10"><div className="flex items-center justify-between mb-3 sm:mb-4"><h3 className="text-lg sm:text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><TrendingUp size={20} className="text-[#00A651]"/> Trending Now</h3><Button variant="ghost" size="sm" onClick={() => navigate(makeHash('shop', undefined, { trending: 'true' }))} className="text-[#006233] dark:text-[#00A651] text-xs sm:text-sm">View All <ChevronRight size={14}/></Button></div><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">{trending.map(p => <ProductCard key={p.id} product={p} onQuickView={onQuickView}/>)}</div></section>)}
      {/* New Arrivals */}{newArrivals.length > 0 && (<section className="mb-8 sm:mb-10"><div className="flex items-center justify-between mb-3 sm:mb-4"><h3 className="text-lg sm:text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><Zap size={20} className="text-[#C5A028]"/> New Arrivals</h3><Button variant="ghost" size="sm" onClick={() => navigate(makeHash('new'))} className="text-[#006233] dark:text-[#00A651] text-xs sm:text-sm">View All <ChevronRight size={14}/></Button></div><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">{newArrivals.map(p => <ProductCard key={p.id} product={p} onQuickView={onQuickView}/>)}</div></section>)}
      {/* Best Sellers */}{bestSellers.length > 0 && (<section className="mb-8 sm:mb-10"><div className="flex items-center justify-between mb-3 sm:mb-4"><h3 className="text-lg sm:text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><ThumbsUp size={20} className="text-[#006233]"/> Best Sellers</h3></div><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">{bestSellers.map(p => <ProductCard key={p.id} product={p} onQuickView={onQuickView}/>)}</div></section>)}
      {/* Testimonials */}<section className="mb-8 sm:mb-10"><h3 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><Users size={20} className="text-[#006233]"/> What Our Customers Say</h3><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">{TESTIMONIALS.map((t, i) => (<Card key={i} className="p-4 border-t-2 border-t-[#C5A028]"><div className="flex items-center gap-3 mb-3"><img src={U(t.avatar, 80)} alt={t.name} className="w-10 h-10 rounded-full object-cover" onError={(e) => imgFallback(e, t.name)}/><div><p className="font-semibold text-sm" style={FB}>{t.name}</p><p className="text-xs text-[#C5A028]">{t.role}</p></div></div><StarRating rating={t.rating} size={12}/><p className="mt-2 text-sm text-muted-foreground">&ldquo;{t.text}&rdquo;</p></Card>))}</div></section>
      {/* Blog */}<section className="mb-8 sm:mb-10"><div className="flex items-center justify-between mb-3 sm:mb-4"><h3 className="text-lg sm:text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-slate-50" style={FH}><BookOpen size={20} className="text-[#006233]"/> From the Blog</h3><Button variant="ghost" size="sm" onClick={() => navigate(makeHash('blog'))} className="text-[#006233] dark:text-[#00A651] text-xs sm:text-sm">All Posts <ChevronRight size={14}/></Button></div><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">{BLOGS.map((b) => (<Card key={b.id} className="overflow-hidden group cursor-pointer hover:shadow-lg transition-shadow"><div className="aspect-video overflow-hidden"><ProductImage src={U(b.img, 400)} alt={b.title} seed={b.img} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" /></div><div className="p-3 space-y-1"><p className="text-xs text-[#C5A028] font-medium">{b.date} &middot; {b.author}</p><p className="font-semibold text-sm" style={FB}>{b.title}</p><p className="text-xs text-muted-foreground line-clamp-2">{b.excerpt}</p></div></Card>))}</div></section>
...
export default function BachatBazarApp() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      {/* Main content */}
      <main ref={mainRef} className="flex-1 max-w-7xl mx-auto w-full px-3 sm:px-4 py-4 sm:py-6">{renderView()}</main>
